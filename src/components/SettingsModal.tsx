...
export function SettingsModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    let initialSettings: LauncherSettings = {
      ramAllocation: 4096,
      customJavaPath: '',
      jvmArgs: '-XX:+UseG1GC -Dsun.rmi.dgc.server.gcInterval=2147483646',
      optionalMods: {},
    ...
    if (stored) {
      initialSettings = { ...initialSettings, ...JSON.parse(stored) };
    }
    ...
    if (window.hexloaderDesktop) {
      window.hexloaderDesktop.getSettings().then(desktopSettings => {
        setSettings(s => ({
          ...s,
          nickname: desktopSettings.nickname || s.nickname || 'HexPilot'
        }));
      });
    }
  }, []);
    ...
    if (window.hexloaderDesktop) {
      try {
        await window.hexloaderDesktop.updateSettings({
          ...
          memoryMb: settings.ramAllocation
        });
      } catch (err) {
        console.error("Failed to save settings to desktop storage:", err);
      }
    }
    onClose();
